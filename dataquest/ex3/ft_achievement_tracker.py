...
def main() -> None:
    ...
    achievements = ['Crafting Genius', 'World Savior', 'Master Explorer', 'Collector Supreme', 
                    'Untouchable', 'Boss Slayer', 'Strategist', 'Speed Runner', 'Survivor',
                    'Treasure Hunter', 'Unstoppable', 'Hidden Path Finder', 'First Steps', 'Sharp Mind']
    players_lst = {'Alice': set(), 'Bob': set(), 'Charlie': set(), 'Dylan': set()}
    ...
    for player, medals in players_lst.items():
        others = set.union(*(s for name, s in players_lst.items() if name != player))
        print(f"Only {player} has: {medals.difference(others)}")

    print()
    for player in players_lst:
        print(f"{player} is missing: {set(achievements).difference(players_lst[player])}")
